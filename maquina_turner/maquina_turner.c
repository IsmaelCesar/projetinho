...
static char string[N] ="KSK(KSK)(SKS)KK\0";
//char string[60] ="KSK(KSK)(SKS)KK\0";
//char string[60] ="K(SKS)(KSS)K\0";
static typedef struct  Celula{
	char tipo;
	char mark;
	struct Celula *fesq;
	struct Celula *fdir;
...
/*Estrutura que cont�m um ponteiro que aponta para a raiz da freelist
typedef struct Freelist{
	struct Celula *fl_pointer;
}Freelist;
*/

static Celula heap[H];
 ...
 * */
static typedef struct NodePilha{
	struct Celula *dado;
 ...
 * */
static Celula *cria_combinador(char *entrada){
	Celula *combinador;
...
/*Procedimento cria uma c�lula do tipo aplica��o(@) com
 * os filhos da direita e da esquerda nulos
 * */
static Celula *cria_aplicacao(){
	Celula *aplicacao;
...
static void casa_parenteses(char* array1, int* p) {
    int paren = 1;
    int c = *p;
    while (paren != 0) {
        switch (array1[c]) {
 ...
 * */
static Celula* cria_grafo(char *entrada){
 ...
 * */
static void imprime_grafo_para_string(Celula *cel){
...
static void imprime_grafo(Celula *cel){
 ...
 * K a b -> A
 * */
static void reduz_K(Celula *grafo){
...
/*Procedimento efetua a redu��o do combinador S segundo a regra
 * S a b c -> a c ( b c )
 * */
static void reduz_S(Celula *grafo){
	//Buscando argumentos
	pop();//Desempilha S
...
/*Procedimento empilha c�lulas d�s de a raiz at� o elemento mais externo mais
 * a esquerda
 * */
static void empilha_grafo(Celula *grafo){
	Celula *g = grafo;
	while(g != NULL){
		push(g);
		g = g->fesq;
