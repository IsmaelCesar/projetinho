...
//char string[N] = "S(C(C'=I0)1)(S'+I(B(Y(B'S(C'I(C'=I0)1)(B'(S'+)I(C'BI(C'-I1)))))(C'-I1)))20\0";//fab2
//char string[N] = "S(C(C=I0)1)(S+I(B(Y(BS(CI(C=I0)1)(B(S+)I(CBI(C-I1)))))(C-I1)))20\0";//fab2
//fibonacci
 ...
 * */
void push(Celula *c){
	//++ vem antes pois o topo comeÁa em -1
	if(topo < P){
		pilha[++topo] = c;
	}
	else{
 ...
 * Este procedimento criar· combinadores e operadores lÛgicos aritmÈticos
 * O n˙mero ser· utilizado no campo tipo durante a reduÁ„o do grafo
 * CodificaÁ„o de FFF00 a FFFFF
 * */
 ...
 * O n˙mero ser· utilizado no campo tipo durante a reduÁ„o do grafo
 * Contando quan dÌgito esta sendo computado para a criaÁ„o do valor
...
/*Procedimento auxiliar para alocar alocar um espaÔøΩo na heap e
...
/*Procedimento auxiliar para verificar se o caractere avaliado È um
...
/*Procedimento auxiliar para converter o valor inteiro de uma cÈlula para caractere
 * */
...
void reduz_K(Celula *grafo){
	...
	newA->fdir = a->fdir;
	newA->fesq = a->fesq;
	//Adicionou verificaÔøΩÔøΩo do pai
	//if(topo != NULL){
	if(topo >= t_eval){
		pai = pilha[topo];
	}
...
/*Procedimento efetua a redu√ßao do combinador I no grafo segundo a regra
 *  I a -> a
 * */
 ...
 * + a b = (eval a) + (eval b)
 * Onde eval È a reduÁ„o da sub arvore do argumento do operador
 * */
...
int main(){
	...
	clock_t tic = clock();
	printf("\n");
	printf("Tempo Cria√ß√£o do grafo = %lf", (double)((tic - toc)/clk_ps));
	...
	printf("\n");
	printf("Tempo Redu√ß√£o grafo = %lf", (double)((b - a)/clk_ps));
	printf("\nNumero de itera√ß√µes = %d\n",iterations);
